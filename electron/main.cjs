...
function captureSettingsPath() {
  return path.join(app.getPath('appData'), 'NEXUS', 'capture-settings.json');
}

function readCaptureSettings() {
  const stablePath = captureSettingsPath();
  const legacyPath = path.join(app.getPath('userData'), 'capture-settings.json');
  for (const settingsPath of [...new Set([stablePath, legacyPath])]) {
    try {
...
function recordFromFile(filePath) {
  ...
  const match = name.match(/_((?:NX|AC)-[A-Z0-9-]+)\.png$/i);
...
function showBackgroundChatNotification(win, payload, force = false) {
  ...
  const author = String(payload?.author || 'NEXUS pilot').replace(/[\r\n]/g, ' ').slice(0, 48);
  ...
  const notification = new Notification({
    ...
    body,
    icon: path.join(__dirname, '..', 'dist', 'nexus.ico'),
    silent: false,
  });
...
async function takeEvidenceScreenshot(source = 'hotkey') {
  ...
  try {
    ...
    const filePath = path.join(directory, `NEXUS_${stamp}${matchSuffix}.png`);
...
function isCapturePath(filePath) {
  const root = path.resolve(captureRoot()) + path.sep;
  return typeof filePath === 'string' && path.resolve(filePath).startsWith(root);
}
...
app.whenReady().then(() => {
  ...
  app.setAppUserModelId('com.nexus.sccompetitive');
  ...
  ipcMain.handle('list-local-captures', (_event, requestedMatchId) => {
    ...
    const files = findCaptureFiles(root)
  ...
  ipcMain.handle('verify-rsi-profile', async (_event, handle, code) => {
    ...
    if (!/^NEXUS-[A-Z0-9-]{8,24}$/.test(cleanCode)) {
    ...
    try {
      const response = await fetch(`https://robertsspaceindustries.com/citizens/${encodeURIComponent(cleanHandle)}`, {
        headers: { 'User-Agent': 'NEXUS-Desktop/0.3-alpha profile-verification' },
