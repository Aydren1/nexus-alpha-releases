...
Deno.serve(async (request) => {
  ...
  if (!/^[A-Za-z0-9_-]{3,32}$/.test(handle) || !/^NEXUS-[A-Z0-9-]{8,24}$/.test(code)) {
  ...
  try {
    const dossierResponse = await fetch(`https://robertsspaceindustries.com/citizens/${encodeURIComponent(handle)}`, {
      headers: { "User-Agent": "NEXUS-Alpha/0.2 RSI-public-profile-verification" },
  ...
  if (profileError) {
    ...
    return reply({ ok: false, reason: duplicate ? "That RSI account is already linked to another NEXUS account." : "Could not activate the profile." }, duplicate ? 409 : 500);
  }
