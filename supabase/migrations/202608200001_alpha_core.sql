...
create table public.matches (
  id uuid primary key default gen_random_uuid(),
  public_id text not null unique default ('NX-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 8))),
  format public.queue_format not null,
...
begin
  ...
  loop
    ...
    insert into public.notifications(user_id, title, body)
      select mp.user_id, 'MATCH FOUND', 'Your NEXUS match room is ready.' from public.match_players mp where mp.match_id = created_match;
...
select cron.schedule('nexus-generate-weekly-tournaments', '5 5 * * 1', 'select public.ensure_weekly_tournaments(8)');
select cron.schedule('nexus-finalize-match-results', '*/5 * * * *', 'select public.finalize_due_match_results()');
select cron.schedule('nexus-small-matchmaker', '* * * * *', 'select public.run_small_matchmaker()');
