...
function WindowTitleBar({showBrand = true}:{showBrand?:boolean}) {
  ...
  return <header className="nexus-titlebar">
    <div className="nexus-titlebar-drag" onDoubleClick={()=>void toggleMaximize()}>
      {showBrand && <><img src="./nexus-icon-32.png" alt=""/><b>NEXUS</b><span>STAR CITIZEN COMPETITIVE</span></>}
    </div>
    <div className="nexus-window-controls">
      <button aria-label="Minimize NEXUS" onClick={()=>void desktop.windowMinimize()}><Minus/></button>
      <button aria-label={maximized?'Restore NEXUS':'Maximize NEXUS'} onClick={()=>void toggleMaximize()}>{maximized?<Minimize2/>:<Maximize2/>}</button>
      <button className="window-close" aria-label="Close NEXUS" onClick={()=>void desktop.windowClose()}><X/></button>
    </div>
  </header>;
}
...
function App() {
  ...
  if (booting) return <AppFrame><div className="alpha-boot"><Orbit size={34}/><b>NEXUS ALPHA</b><span>CONNECTING TO COMPETITIVE NETWORK</span></div></AppFrame>;

  if (!account?.verified) {
    return <AppFrame><AuthFlow onComplete={completeAccount} /></AppFrame>;
  }

  return <AppFrame showTitleBrand={false}><NexusApp account={account} onAccountUpdate={updateAccount} onLogout={logout} /></AppFrame>;
}

function NexusApp({ account, onAccountUpdate, onLogout }: { account: Account; onAccountUpdate: (account: Account) => Promise<Account>; onLogout: () => Promise<void> }) {
  ...
  const [captureSetupComplete, setCaptureSetupComplete] = useState(() => !window.nexusDesktop || Boolean(localStorage.getItem('nexus-capture-setup-v1')));
  const [captureSetupResolved, setCaptureSetupResolved] = useState(() => !window.nexusDesktop);
  ...
  useEffect(()=>{
    ...
    void backend.subscribeIncomingChat(message=>{
      if(active)void window.nexusDesktop?.showBackgroundChatNotification({author:message.author,channel:message.channel,body:message.text});
    }).then(channel=>{
  ...
  useEffect(()=>{
    ...
    const remove=window.nexusDesktop.onOpenChatChannel(channel=>openChat(channel));
    return remove;
  },[]);
  useEffect(()=>{
    const updateBackgroundState=()=>document.documentElement.classList.toggle('nexus-background',document.hidden);
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--nexus-font-scale', String(fontScale));
    if (window.nexusDesktop) {
      document.documentElement.style.removeProperty('zoom');
      window.nexusDesktop.setUiScale(fontScale);
    } else {
  ...
  useEffect(() => {
    if (!window.nexusDesktop) return;
    void window.nexusDesktop.getCaptureSettings().then((settings) => {
  ...
  useEffect(() => {
    if (!window.nexusDesktop) return;
    const removeComplete = window.nexusDesktop.onCaptureComplete((result) => {
      if (result.ok && result.record) notify(`Evidence saved locally${result.record.matchId ? ` for ${result.record.matchId}` : ''}.`);
    });
    const removeFailed = window.nexusDesktop.onCaptureFailed((result) => notify(result.reason || 'Screenshot capture failed.'));
  ...
  if (!captureSetupComplete && window.nexusDesktop) {
  ...
  return (
    <div className={`app-shell theme-${theme}`}>
      <aside className="rail">
        <button className="brand" onClick={() => setPage('Play')} aria-label="Nexus home">
          <span className="brand-mark"><img src="./nexus-icon-512-cyan.png" alt="" /></span>
          <span>NEXUS</span>
        </button>
...
function CaptureSetup({ onComplete }: { onComplete: () => void }) {
  ...
  const [folder, setFolder] = useState('Pictures\\NEXUS\\Captures');
  ...
  useEffect(() => {
    if (step !== 1 || !window.nexusDesktop) return;
    const remove = window.nexusDesktop.onCaptureComplete(result => {
      if (result.ok && result.record) { setTestCapture(result.record); setError(''); }
    });
    const removeFailed = window.nexusDesktop.onCaptureFailed(result => setError(result.reason || 'The test capture failed.'));
    return () => { remove(); removeFailed(); };
  }, [step]);

  const configure = async () => {
    setBusy(true); setError('');
    const result = await window.nexusDesktop!.configureCaptureShortcut(accelerator);
    setBusy(false);
  ...
  return <div className="capture-setup-shell">
    <div className="capture-setup-brand"><span className="brand-mark"><img src="./nexus-icon-512.png" alt=""/></span><b>NEXUS</b><span>INITIAL PILOT SETUP</span></div>
    <section className="capture-setup-card">
      ...
      {step===0&&<>
        ...
        <p>NEXUS can take a full-resolution screenshot of the display under your cursor—even while Star Citizen is focused. Captures stay on this PC until you choose to submit them.</p>
        ...
        <div className="setup-note"><ShieldCheck/><span><b>PRIVATE BY DEFAULT</b><small>Nothing is uploaded automatically. NEXUS only stores PNG files in your Pictures folder.</small></span></div>
      ...
      {step===1&&<>
        ...
        <div className={`capture-test-zone ${testCapture?'success':''}`}>{testCapture?<><img src={testCapture.thumbnail} alt="Test screenshot preview"/><div><ShieldCheck/><b>CAPTURE SAVED</b><span>{new Date(testCapture.takenAt).toLocaleString()}</span></div></>:<><Crosshair size={42}/><b>WAITING FOR HOTKEY</b><span>NEXUS can remain in the background.</span></>}</div>
        <div className="setup-actions"><button className="secondary" onClick={()=>void window.nexusDesktop?.captureEvidenceNow()}><Camera/> TAKE TEST WITH BUTTON</button><button className="primary" disabled={!testCapture} onClick={()=>setStep(2)}>CONTINUE <ChevronRight/></button></div>
        {error&&<div className="setup-error"><AlertTriangle/>{error}</div>}
      </>}
      {step===2&&<>
        ...
        <p>When a NEXUS match is active, new screenshots are linked to that match automatically. Open Match Center to review them before confirming or disputing a result.</p>
        <button className="primary setup-next" onClick={finish}>ENTER NEXUS <ChevronRight/></button>
      </>}
    </section>
  </div>;
}
...
function EvidenceDrawer({matchId,close,notify,reconfigure}:{matchId:string;close:()=>void;notify:(message:string)=>void;reconfigure:()=>void}) {
  ...
  const refresh=async()=>{
    if(!window.nexusDesktop){setLoading(false);return;}
    const [nextCaptures,nextSettings]=await Promise.all([window.nexusDesktop.listLocalCaptures(matchId||undefined),window.nexusDesktop.getCaptureSettings()]);
    setCaptures(nextCaptures);setSettings(nextSettings);setLoading(false);
  };
  useEffect(()=>{void refresh();const remove=window.nexusDesktop?.onCaptureComplete(()=>void refresh());return()=>remove?.()},[matchId]);
  const capture=async()=>{const result=await window.nexusDesktop?.captureEvidenceNow();if(!result?.ok)notify(result?.reason||'Capture failed.');};
...
function AuthFlow({ onComplete }: { onComplete: (account: Account) => void }) {
  ...
  const [code] = useState(() => `NEXUS-${crypto.randomUUID().slice(0, 4).toUpperCase()}-${crypto.randomUUID().slice(0, 4).toUpperCase()}`);
  ...
  const continueAccount = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(accountError instanceof Error ? accountError.message : 'Could not access the NEXUS account service.');
    } finally {
  ...
  const verify = async () => {
    ...
    try {
      ...
      const dossierCheck: Promise<{ ok: boolean; reason?: string }> = window.nexusDesktop
        ? window.nexusDesktop.verifyRsiProfile(handle.trim(), code)
  ...
  return <div className="auth-shell">
    ...
    <header className="auth-header"><div className="brand"><span className="brand-mark"><img src="./nexus-icon-512.png" alt=""/></span><span>NEXUS</span></div><span>STAR CITIZEN COMPETITIVE NETWORK</span></header>
    <section className="auth-story">
      <span className="season-tag"><span/> VERIFIED COMPETITION</span>
      <h1>ONE CITIZEN.<br/><em>ONE RECORD.</em></h1>
      <p>Every NEXUS competitor is connected to a public RSI identity. Verified pilots protect matchmaking, rankings, and tournament integrity.</p>
    ...
    <section className="auth-card">
      ...
      {step === 'account' ? <>
        <div className="auth-title"><span className="eyebrow">ACCESS NEXUS</span><h2>{mode === 'register' ? 'CREATE YOUR ACCOUNT' : 'WELCOME BACK'}</h2><p>{mode === 'register' ? 'Your competitive identity starts here.' : 'Sign in to continue your competitive record.'}</p></div>
        ...
        <p className="auth-legal">By continuing, you agree to the NEXUS Terms of Service and Competitive Code.</p>
      </> : <>
        ...
        <div className="verify-steps">
          ...
          <div><span>03</span><p>Enter your exact RSI handle, then let NEXUS check the public dossier.</p><label>RSI HANDLE<input value={handle} onChange={e=>setHandle(e.target.value)} placeholder="Example: ArcRunner"/></label></div>
        </div>
        {error && <div className="auth-error"><AlertTriangle size={15}/>{error}</div>}
    ...
    <footer className="auth-footer"><span>NEXUS ALPHA 7.14</span><span>SYSTEM STATUS <i/> OPERATIONAL</span></footer>
  </div>;
}
  ...
  return <>
    ...
    <div className="dashboard-grid">
      <section className="panel recent-panel">
        ...
        {activityLoading?<LiveEmpty icon={<Orbit/>} title="LOADING MATCH HISTORY" detail="Reading your verified NEXUS match records."/>:myMatches.length===0?<LiveEmpty icon={<Swords/>} title="NO MATCHES RECORDED" detail="Your queued and verified matches will appear here."/>:myMatches.map(m => <div className="match-row" key={m.id}>
...
function MatchesPage({openPanel,openEvidence}:{openPanel:(mode:PanelMode,title?:string)=>void;openEvidence:()=>void}) {
  ...
  return <div className="standard-page">
    <PageTitle eyebrow="MATCH CENTER" title="YOUR MATCHES" description="Only matches linked to your verified NEXUS account appear here." action={<button className="secondary" onClick={()=>openPanel('match-rules')}><ClipboardCheck size={17}/> MATCH RULES</button>} />
...
function AccessRestricted({account,access,onLogout}:{account:Account;access:PlatformAccess;onLogout:()=>Promise<void>}){
  return <div className="moderation-lock"><ShieldCheck size={42}/><span className="eyebrow">ACCOUNT ENFORCEMENT</span><h1>ACCESS SUSPENDED</h1><p><b>{account.handle}</b>, this NEXUS account is currently banned.</p><div><small>REASON</small><strong>{access.banReason||'Platform rules violation'}</strong><small>EXPIRATION</small><strong>{access.banExpiresAt?new Date(access.banExpiresAt).toLocaleString():'Permanent'}</strong></div><button className="secondary" onClick={()=>void onLogout()}><LogOut size={15}/> SIGN OUT</button></div>;
}
...
function ProfileDrawer({account,close,save,onLogout,notify}:{account:Account;close:()=>void;save:(account:Account)=>Promise<Account>;onLogout:()=>Promise<void>;notify:(s:string)=>void}) {
  ...
  return <div className="drawer-backdrop" onMouseDown={close}><aside className="party-drawer profile-drawer" onMouseDown={e=>e.stopPropagation()}>
    ...
    <button className="signout-button" onClick={()=>void onLogout()}><LogOut size={15}/> SIGN OUT OF NEXUS</button>
  </aside></div>;
}
...
function UtilityPanel({panel,account,access,selectedQueue,members,setPage,close,notify,theme,setTheme,fontScale,setFontScale,backgroundChatNotifications,setBackgroundChatNotifications,onCommunityEventCreated,hasUnreadNotifications,cloudNotifications,markNotificationsRead}:{panel:PanelState;account:Account;access:PlatformAccess;selectedQueue:typeof queues[number];members:PartyMember[];setPage:(page:Page)=>void;close:()=>void;notify:(message:string)=>void;theme:ThemeMode;setTheme:(theme:ThemeMode)=>void;fontScale:number;setFontScale:(scale:number)=>void;backgroundChatNotifications:boolean;setBackgroundChatNotifications:(enabled:boolean)=>void;onCommunityEventCreated:()=>void;hasUnreadNotifications:boolean;cloudNotifications:CloudNotification[];markNotificationsRead:(notificationId?:string)=>Promise<void>}){
  ...
  const searchResults=[...nav,{label:'Admin' as Page,icon:ShieldCheck}].map(item=>({name:item.label,type:'NEXUS section',page:item.label})).filter(item=>!query||item.name.toLowerCase().includes(query.toLowerCase()));
  ...
  const body=(()=>{
    if(panel.mode==='search')return <><label className="utility-search"><Search/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Find a NEXUS section"/></label><div className="utility-results">{searchResults.map(result=><button key={result.name} onClick={()=>{setPage(result.page);close()}}><span className="avatar">{result.name.slice(0,2).toUpperCase()}</span><div><b>{result.name}</b><small>{result.type}</small></div><ChevronRight/></button>)}</div><p className="utility-copy">Pilot, organization, and event directory search will be enabled when its live index is ready.</p></>;
    ...
    if(panel.mode==='settings')return <div className="utility-form">
      ...
      {window.nexusDesktop&&<button className="secondary background-chat-test" disabled={!backgroundChatNotifications} onClick={()=>void window.nexusDesktop?.testBackgroundChatNotification()}>MINIMIZE &amp; TEST CHAT ALERT</button>}
      <label className="toggle-row">MATCH READY SOUND<input type="checkbox" defaultChecked/></label>{window.nexusDesktop&&<label className="toggle-row">LAUNCH AT WINDOWS STARTUP<input type="checkbox"/></label>}
  ...
  return <div className="utility-backdrop" onMouseDown={close}><section className={`utility-panel ${panel.mode==='event-details'?'bracket-panel':''}`} onMouseDown={event=>event.stopPropagation()}><div className="utility-head"><div><span className="eyebrow">NEXUS COMMAND</span><h2>{titles[panel.mode]}</h2></div><button onClick={close}><X/></button></div><div className="utility-body">{body}</div></section></div>;
}
