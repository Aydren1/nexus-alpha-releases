...
const packagedPathMarker = path.join(releaseRoot, '.nexus-packaged-app-path');
const packagedApp = fs.existsSync(packagedPathMarker)
  ? fs.readFileSync(packagedPathMarker, 'utf8').trim()
  : path.join(releaseRoot, 'NEXUS-win32-x64');
...
const required = ['/package.json', '/electron/main.cjs', '/electron/preload.cjs', '/dist/index.html', '/dist/nexus.ico', '/dist/nexus-icon-512.png', '/dist/nexus-icon-512-cyan.png'];
...
if (packageJson.version !== '0.5.0-alpha.7.14') throw new Error(`Unexpected packaged version: ${packageJson.version}`);
...
for (const marker of ['requestSingleInstanceLock', "app.getPath('appData')", "'NEXUS', 'capture-settings.json'", 'registered: Boolean(captureShortcut)']) {
